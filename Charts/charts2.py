...
def merge_and_sort_data(valid_currents, airtable_data, weeks_in_studio):
    ...
    # Final sort:
    # a. Highest NUMBER OF PLAYS (COUNT, capped at 2 per show)
    # b. Local (local → detroit → cancon → other)
    # c. Label priority (self → indie → major)
    # d. Number of weeks in studio (fewer weeks first = newer titles win ties)
    sorted_data = merged_data.sort_values(
        by=['COUNT', 'LOCAL_PRIORITY', 'LABEL_PRIORITY', 'WEEKS_IN_STUDIO'],
...
def main():
    ...
    sorted_data = merge_and_sort_data(valid_currents, airtable_data, weeks_in_studio)
